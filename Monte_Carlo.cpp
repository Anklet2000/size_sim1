...
void particle_init() {
	...
	while (i < N) {
		...
		Y[i] = rand() % 24 + rand() / (double)RAND_MAX - 12.0;
		if (X[i] < R - 6.0 || X[i] > 6.0 - R || Y[i] < R - 12.0 || Y[i] > 12.0 - R) {
			flag = 0;
		}
		if (flag == 1) {
			for (int j = 0; j < i; j++) {
...
void down() {
	...
	std::string filename = "sim1data.txt";
	...
	for (int k = 0; k < 10000; k++) {
		...
		while (i < N) {
			...
			if (X2[i] < R - 6.0 || X2[i] > 6.0 - R || Y2[i] < R - 12.0 || Y2[i] > 12.0 - R) {
				flag = 0;
			}
			if (flag == 1) {
				for (int j = 0; j < n; j++) {
